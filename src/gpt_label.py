...
class ZotGPTLabeler:
    ...
    def __init__(self, api_key, base_url=ZOTGPT_BASE_URL):
        if not api_key:
            raise ValueError("ZotGPT API key not found. Please set ZOTGPT_API_KEY in .env file")
        ...
        try:
            ...
            try:
                ...
                if label not in [0, 1, 2, 3, 4]:
                    print(f"Warning: Invalid label {label} for article {article_id}. Defaulting to 2 (neutral)")
                    label = 2
            except ValueError:
                print(f"Warning: Could not parse label '{label_text}' for article {article_id}. Defaulting to 2 (neutral)")
                label = 2
    ...
    def label_articles_batch(self, articles, start_idx=0, count=50, delay=1.0):
        ...
        for idx in range(start_idx, end_idx):
            ...
            if 'error' not in result:
                label_names = ['Negative', 'Leaning Negative', 'Neutral', 'Leaning Positive', 'Positive']
