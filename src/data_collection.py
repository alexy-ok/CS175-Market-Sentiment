...
class GuardianAPICollector:
    ...
    def __init__(self, api_key):
        if not api_key:
            raise ValueError(
                "Guardian API key is required. Please set GUARDIAN_API_KEY in .env file"
            )
    ...
    def collect_articles(self, from_date, to_date, max_articles=1000):
        ...
        while len(articles) < max_articles:
            ...
            print(
                f"Collected {len(articles)} articles (Page {current_page}/{total_pages})"
            )
...
def main():
    try:
        ...
        articles = collector.collect_articles(
            from_date=from_date_str, to_date=to_date_str, max_articles=2000
        )
